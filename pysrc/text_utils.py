"""
utils for cleaning up data:
clean_data: remove non-linguistic tokens, punctuations, and stopwords
"""
...
"didn't", 'different', 'do', 'does', "doesn't", 'doing', 'done', "don't", 'down', 
...
'everywhere', 'ex', 'except', 'f', 'far', 'few', 'ff', 'fifth', 'first', 'five', 
'fix', 'followed', 'following', 'follows', 'for', 'former', 'formerly', 'forth', 
'found', 'four', 'from', 'further', 'furthermore', 'g', 'gave', 'get', 'gets', 
'getting', 'give', 'given', 'gives', 'giving', 'go', 'goes', 'gone', 'got', 
'gotten', 'h', 'had', 'happens', 'hardly', 'has', "hasn't", 'have', "haven't", 
'having', 'he', 'hed', 'hence', 'her', 'here', 'hereafter', 'hereby', 'herein', 
'heres', 'hereupon', 'hers', 'herself', 'hes', 'hi', 'hid', 'him', 'himself', 
'his', 'hither', 'home', 'how', 'howbeit', 'however', 'hundred', 'i', 'id', 'ie', 
'if', "i'll", 'im', 'immediate', 'immediately', 'importance', 'important', 'in', 
...
'ltd', 'm', 'made', 'mainly', 'make', 'makes', 'many', 'may', 'maybe', 'me', 'mean', 
...
'ought', 'our', 'ours', 'ourselves', 'out', 'outside', 'over', 'overall', 'owing', 
'own', 'p', 'page', 'pages', 'part', 'particular', 'particularly', 'past', 'per', 
...
'rd', 're', 'readily', 'really', 'recent', 'recently', 'ref', 'refs', 'regarding', 
...
'says', 'sec', 'section', 'see', 'seeing', 'seem', 'seemed', 'seeming', 'seems', 'seen', 
'self', 'selves', 'sent', 'seven', 'several', 'shall', 'she', 'shed', "she'll", 'shes', 
...
'such', 'sufficiently', 'suggest', 'sup', 'sure\tt', 'take', 'taken', 'taking', 'tell', 
'tends', 'th', 'than', 'thank', 'thanks', 'thanx', 'that', "that'll", 'thats', "that've", 
...
'throughout', 'thru', 'thus', 'til', 'tip', 'to', 'together', 'too', 'took', 'toward', 
...
'v', 'value', 'various', "'ve", 'very', 'via', 'viz', 'vol', 'vols', 'vs', 'w', 'want', 
'wants', 'was', 'wasnt', 'way', 'we', 'wed', 'welcome', "we'll", 'went', 'were', 'werent', 
...
'wont', 'words', 'world', 'would', 'wouldnt', 'www', 'x', 'y', 'yes', 'yet', 'you', 
'youd', "you'll", 'your', 'youre', 'yours', 'yourself', 'yourselves', "you've", 'z', 'zero'}

# stemmer = PorterStemmer()

def clean_data(text):
    text = re.sub(r'https?:.*[\s\r\n]*', UNK_HOLDER, text.lower()) # remove links
    text = re.sub(r'[\r\n\t]', UNK_HOLDER, text)    #remove format tokens
    text = re.sub('&amp', UNK_HOLDER, text)   #remove &amp sign
    text = re.sub(r'[@][a-z]+', UNK_HOLDER, text)   #remove mentions
    text = re.sub(r'[#][a-z]+', UNK_HOLDER, text)   #remove hashtags
    # print "text", text
    tokens = re.split('[ ]+', re.sub(r"[^#@&a-z ]+", UNK_HOLDER, text))    #remove non-words and split
    # substitutes stopword to UNK
    # return [stemmer.stem(t) if t not in STOPWORDS else UNK for t in tokens]
    # print "tokens", tokens
    return [t if t not in STOPWORDS else UNK for t in tokens if len(t) > 1]